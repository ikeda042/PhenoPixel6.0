...
PIXEL_SIZE_UM = 0.065
EXPECTED_POINTS_PER_CELL = 35
DEFAULT_MODE = "HU_aggregation_ratio"
...
class GraphEngineCrud:
    ...
    def _parse_ctrl(text: str) -> float:
        ...
        data = [float(np.sum(row) / len(row)) for row in normalized_rows if row]
    ...
    def _analyze_csv(text: str, ctrl: float | None) -> tuple[float, float | None]:
        ...
        for i in range(0, len(lines), 2):
            ...
            if len(values) < 2:
                continue
            try:
                length = round((float(values[-1]) - float(values[0])) * PIXEL_SIZE_UM, 2)
            except ValueError:
                continue
            try:
        ...
        mean_length = float(np.mean([length for length, _ in cells]))
        if ctrl is None:
            return mean_length, None
        count_below = sum(1 for _, peak in cells if peak < ctrl)
        nagg_rate = count_below / len(cells)
    ...
    def _extract_peak_vectors(cls, text: str) -> list[list[float]]:
        ...
        for i in range(0, len(lines), 2):
            ...
            if len([j for j in values if j != ""]) != EXPECTED_POINTS_PER_CELL:
                continue
            try:
                vector = [float(x) for x in lines[i + 1].split(',') if x.strip()]
            except ValueError:
                continue
            if vector:
    ...
    def _parse_numeric_rows(text: str) -> list[list[float]]:
        ...
        for line in text.strip().splitlines():
            if not line.strip():
                continue
            values = [value for value in line.split(',') if value.strip()]
            if not values:
                continue
            try:
    ...
    def _get_matrix_for_mode(cls, mode: str, text: str) -> np.ndarray:
        vectors = cls._extract_peak_vectors(text) if mode == DEFAULT_MODE else []
        if not vectors:
            vectors = cls._parse_numeric_rows(text)
        if not vectors:
            raise ValueError("No numeric data found in CSV")
        max_len = max(len(row) for row in vectors)
    ...
    def create_heatmap_abs_plot(cls, mode: str | None, content: bytes) -> bytes:
        normalized_mode = cls.normalize_mode(mode)
        matrix = cls._get_matrix_for_mode(normalized_mode, cls._safe_decode(content))
